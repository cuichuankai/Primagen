...
- `src/`: Source code
  ...
  - `mcp/`: MCP (Model Context Protocol) client and manager
  - `providers/`: LLM providers
  - `session/`: Session management
  - `skills/`: Skills loader
  - `subagent/`: Subagent manager
  - `tools/`: Tool registry
  - `vendor/`: Third-party libraries (cJSON, Mongoose)
...
**Note on MCP**: MCP Client is fully implemented with stdio transport. Includes Amap (Gaode Map) MCP server with 7 geographic tools (geocoding, weather, POI search, distance calculation, route planning).
