...
typedef struct LongPollWaiter {
    struct mg_connection* conn;
    struct mg_mgr* mgr;
    struct mg_timer* timer;
    char chat_id[256];
    struct LongPollWaiter* next;
} LongPollWaiter;
...
".group-title{font-size:11px;font-weight:600;color:var(--steel);padding:6px 10px 4px;text-transform:uppercase;letter-spacing:.5px}.sidebar.collapsed .group-title{display:none}.item,.subitem{display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:var(--r-md);cursor:pointer;color:var(--charcoal);margin-bottom:2px;user-select:none;font-size:14px;font-weight:400}"
".item:hover,.subitem:hover{background:var(--surface)}.item.active,.subitem.active{background:var(--surface-soft);color:var(--primary);font-weight:600}.icon{width:18px;height:18px;display:flex;align-items:center;justify-content:center;flex:none}"
".icon svg{width:16px;height:16px;stroke:currentColor;stroke-width:1.9;fill:none;stroke-linecap:round;stroke-linejoin:round}.label{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.sidebar.collapsed .label{display:none}.caret{margin-left:auto;color:var(--steel)}.sidebar.collapsed .caret{display:none}"
".sublist{margin:2px 0 2px 22px;padding-left:10px;border-left:1px solid var(--hairline-soft);display:none}.sublist.open{display:block}.sidebar.collapsed .sublist{display:none!important}"
".icon-btn{width:30px;height:30px;border:1px solid var(--hairline);background:var(--panel);color:var(--charcoal);border-radius:var(--r-md);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:border-color .2s ease,box-shadow .2s ease}.icon-btn svg{width:14px;height:14px;stroke:currentColor;stroke-width:2;fill:none;stroke-linecap:round;stroke-linejoin:round}.icon-btn:hover{border-color:var(--primary);box-shadow:0 0 0 3px rgba(86,69,212,.14)}"
".content{flex:1;display:flex;flex-direction:column;min-width:0;height:100vh}.topbar{position:sticky;top:0;z-index:9;height:54px;display:flex;align-items:center;justify-content:space-between;padding:8px 18px;background:rgba(255,255,255,.88);backdrop-filter:blur(12px);border-bottom:1px solid var(--hairline-soft);box-shadow:0 1px 0 var(--hairline-soft)}"
".path{font-size:13px;color:var(--muted);background:var(--panel-2);padding:7px 10px;border-radius:10px;border:1px solid var(--line)}.path strong{color:var(--text)}.menu-btn,.theme-btn{border:1px solid var(--line);background:var(--panel-2);color:var(--text);border-radius:10px;padding:7px 10px;cursor:pointer;display:inline-flex;align-items:center;gap:8px}.theme-btn:hover{border-color:var(--primary);box-shadow:0 0 0 3px color-mix(in srgb,var(--primary) 18%, transparent),0 10px 22px rgba(79,107,255,.2)}.menu-btn{display:none}"
...
".row{display:flex;gap:12px;flex-wrap:wrap}.card{background:linear-gradient(155deg,color-mix(in srgb,var(--panel) 90%, transparent),var(--panel));border:1px solid color-mix(in srgb,var(--line) 88%, var(--primary));border-radius:16px;padding:14px;box-shadow:var(--shadow);margin-bottom:12px}"
".item,.subitem,.card,button,input,textarea,select,.theme-btn,.icon-btn{transition:all .2s ease}.card:hover{transform:translateY(-2px);box-shadow:0 18px 36px rgba(30,41,59,.14)}"
...
".overview-hero{display:grid;grid-template-columns:1.4fr 1fr;gap:12px;margin-bottom:12px}.hero-panel{background:var(--brand-navy);border-radius:var(--r-lg);padding:24px 28px;position:relative;overflow:hidden;box-shadow:var(--shadow-mockup);min-height:180px}.hero-panel::before{content:'';position:absolute;right:-80px;top:-80px;width:280px;height:280px;border-radius:50%;background:radial-gradient(circle,rgba(123,63,242,.55) 0%,transparent 60%);pointer-events:none}.hero-panel::after{content:'';position:absolute;left:20px;bottom:-50px;width:160px;height:160px;border-radius:50%;background:radial-gradient(circle,rgba(42,157,153,.45) 0%,transparent 60%);pointer-events:none}.hero-dot{position:absolute;border-radius:50%;pointer-events:none;opacity:.85;z-index:0}.hero-dot.pink{width:14px;height:14px;background:#ff64c8;top:18px;right:120px;box-shadow:0 0 12px rgba(255,100,200,.5)}.hero-dot.teal{width:10px;height:10px;background:#2a9d99;top:50px;right:60px;box-shadow:0 0 10px rgba(42,157,153,.45)}.hero-dot.green{width:12px;height:12px;background:#1aae39;bottom:30px;right:180px;box-shadow:0 0 10px rgba(26,174,57,.4)}.hero-dot.yellow{width:9px;height:9px;background:#f5d75e;top:30px;right:200px;box-shadow:0 0 10px rgba(245,215,94,.45)}.hero-dot.purple{width:11px;height:11px;background:#7b3ff2;bottom:55px;right:100px;box-shadow:0 0 10px rgba(123,63,242,.5)}.hero-dot.orange{width:8px;height:8px;background:#dd5b00;top:70px;right:160px;box-shadow:0 0 8px rgba(221,91,0,.4)}.hero-spark{position:absolute;pointer-events:none;z-index:0;border-radius:1px;opacity:.3}.hero-spark.s1{width:60px;height:1px;background:rgba(255,255,255,.4);top:40px;right:40px;transform:rotate(-30deg)}.hero-spark.s2{width:40px;height:1px;background:rgba(255,255,255,.25);top:80px;right:90px;transform:rotate(20deg)}.hero-title{font-size:22px;font-weight:600;color:var(--on-dark);letter-spacing:-.4px;position:relative;z-index:1;line-height:1.2}.hero-sub{font-size:14px;color:var(--on-dark-muted);margin-top:8px;position:relative;z-index:1;line-height:1.5}.quick-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px;position:relative;z-index:1}.quick-actions button{height:34px;padding:0 14px;border-radius:var(--r-md);font-size:13px;font-weight:500;background:rgba(255,255,255,.92);color:var(--ink-deep);border-color:transparent;transition:background .2s ease,box-shadow .2s ease}.quick-actions button:hover{background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.12)}.overview-kpi{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-bottom:12px}.kpi{background:var(--canvas);border:1px solid var(--hairline);border-radius:var(--r-lg);padding:16px 18px;position:relative;overflow:hidden;transition:box-shadow .2s ease,transform .2s ease}.kpi:hover{transform:translateY(-2px);box-shadow:var(--shadow-2)}.kpi::before{content:'';position:absolute;left:0;top:12px;bottom:12px;width:3px;border-radius:0 2px 2px 0}.kpi:nth-child(1)::before{background:var(--success)}.kpi:nth-child(2)::before{background:var(--primary)}.kpi:nth-child(3)::before{background:var(--brand-teal)}.kpi .k{font-size:12px;color:var(--slate);font-weight:500;text-transform:uppercase;letter-spacing:.4px}.kpi .v{font-size:24px;font-weight:600;color:var(--ink-deep);margin-top:6px;letter-spacing:-.5px}.chat-toolbar{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 18px;border-bottom:1px solid var(--hairline);background:var(--panel)}.chat-toolbar .toolbar-actions{display:flex;gap:8px}.chat-toolbar .toolbar-actions .secondary{height:34px;padding:0 12px;border-radius:var(--r-md)}.config-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.field{display:flex;flex-direction:column;gap:6px}.field label{font-size:12px;color:var(--slate)}.status-hint{font-size:12px;color:var(--slate)}.status-hint.dirtyHint{color:#d97706}.status-hint.saved{color:#16a34a}.status-hint.error{color:#dc2626}.op-history{margin-top:12px}.op-history .list-kv{border-top:1px dashed var(--hairline-soft)}@keyframes floatGlow{0%{transform:translate3d(0,0,0)}100%{transform:translate3d(-28px,20px,0)}}@keyframes riseIn{0%{opacity:0;transform:translateY(7px)}100%{opacity:1;transform:translateY(0)}}.view>*{animation:riseIn .36s ease both}.item.active,.subitem.active{box-shadow:0 8px 20px rgba(86,69,212,.12)}"
...
"function getTabChatId(){const k='webui.chat.session_id';let id=sessionStorage.getItem(k);if(!id){id=(window.crypto&&window.crypto.randomUUID)?window.crypto.randomUUID():('tab-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,10));sessionStorage.setItem(k,id)}return 'webui-'+id}"
"function renderChat(){const chatId=(window._chatId&&String(window._chatId))||getTabChatId();window._chatId=chatId;pageRoot.innerHTML='<div class=\"chat-shell\"><div class=\"chat-toolbar\"><div><strong>AI 工作台</strong><div class=\"status-hint\">支持轮询控制与快速重试</div></div><div class=\"toolbar-actions\"><button id=\"stopPollBtn\" class=\"secondary\">停止轮询</button><button id=\"retryBtn\" class=\"secondary\">重试</button><button id=\"clearBtn\" class=\"secondary\">清空</button></div></div><div id=\"chatMessages\" class=\"chat-messages\"><div class=\"chat-empty\">Start chatting with Primagen</div></div><div class=\"chat-compose\"><div class=\"chat-compose-box\"><textarea id=\"msgInput\" class=\"chat-input\" placeholder=\"Type your message...\"></textarea><button id=\"sendBtn\" class=\"send-btn\">发送</button></div></div></div>';const box=document.getElementById('chatMessages');const input=document.getElementById('msgInput');let pollEnabled=true;let lastUserInput='';const clearEmpty=()=>{const e=box.querySelector('.chat-empty');if(e)e.remove()};const send=async(text)=>{const message=(text!==undefined?text:input.value||'').trim();if(!message)return;lastUserInput=message;clearEmpty();appendChatMessage('user',message);input.value='';try{await jpost('/api/chat/send',{chat_id:chatId,message})}catch(e){appendChatMessage('assistant','Send failed: '+(e&&e.message?e.message:'unknown_error'))}};document.getElementById('sendBtn').onclick=()=>send();document.getElementById('retryBtn').onclick=()=>send(lastUserInput);document.getElementById('clearBtn').onclick=()=>{box.innerHTML='<div class=\"chat-empty\">Start chatting with Primagen</div>'};document.getElementById('stopPollBtn').onclick=(e)=>{pollEnabled=!pollEnabled;e.target.textContent=pollEnabled?'停止轮询':'恢复轮询'};input.onkeydown=(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}};if(window._chatTimer)clearTimeout(window._chatTimer);(function longPoll(){if(!pollEnabled||activePath()!=='/chat'){window._chatTimer=setTimeout(longPoll,1200);return}fetch('/api/chat/poll?chat_id='+encodeURIComponent(chatId)).then(r=>{if(r.status===204){window._chatTimer=setTimeout(longPoll,100);return}if(!r.ok){window._chatTimer=setTimeout(longPoll,1200);return}r.json().then(x=>{if(x&&x.message){clearEmpty();appendChatMessage('assistant',x.message)}window._chatTimer=setTimeout(longPoll,100)})}).catch(()=>{window._chatTimer=setTimeout(longPoll,1200)})})()}"
...
static void long_poll_timeout_cb(void* arg) {
    LongPollWaiter* w = (LongPollWaiter*)arg;
    if (!w) return;
    WebUIChannelData* data = (WebUIChannelData*)w->conn->fn_data;
    pthread_mutex_lock(&data->lock);
    LongPollWaiter* prev = NULL;
    LongPollWaiter* cur = data->long_poll_head;
    while (cur) {
        if (cur == w) {
            if (prev) prev->next = cur->next;
            else data->long_poll_head = cur->next;
            break;
        }
        prev = cur;
        cur = cur->next;
    ...
    if (cur) {
        mg_http_reply(w->conn, 204, "", "");
    }
    free(w);
}

static void wake_long_poll_waiters(WebUIChannelData* data, const char* chat_id, const char* content) {
    ...
    while (cur) {
        if (strcmp(cur->chat_id, chat_id) == 0) {
            if (cur->timer) {
                mg_timer_free(&data->mgr.timers, cur->timer);
            }
            cJSON* resp = cJSON_CreateObject();
            cJSON_AddStringToObject(resp, "chat_id", chat_id);
            cJSON_AddStringToObject(resp, "message", content);
            char* s = cJSON_PrintUnformatted(resp);
            cJSON_Delete(resp);
            if (s) {
                mg_http_reply(cur->conn, 200, "Content-Type: application/json\r\n", "%s", s);
                free(s);
            } else {
                mg_http_reply(cur->conn, 500, "Content-Type: application/json\r\n", "{\"error\":\"encode_failed\"}");
            }
            if (prev) prev->next = cur->next;
            else data->long_poll_head = cur->next;
            LongPollWaiter* old = cur;
            cur = cur->next;
            free(old);
            continue;
        }
...
static void enqueue_reply(WebUIChannelData* data, const char* chat_id, const char* content) {
    ...
    pthread_mutex_lock(&data->lock);
    LongPollWaiter* w = data->long_poll_head;
    bool has_waiter = false;
    while (w) {
        if (strcmp(w->chat_id, chat_id) == 0) { has_waiter = true; break; }
        w = w->next;
    }
    pthread_mutex_unlock(&data->lock);

    if (has_waiter) {
        wake_long_poll_waiters(data, chat_id, content);
        return;
    }
...
static void handle_chat_poll(struct mg_connection* c, struct mg_http_message* hm, WebUIChannelData* data) {
    ...
    LongPollWaiter* w = malloc(sizeof(LongPollWaiter));
    if (!w) {
        send_json(c, 500, "{\"error\":\"oom\"}");
        return;
    }
    w->conn = c;
    w->mgr = &data->mgr;
    strncpy(w->chat_id, chat_id, sizeof(w->chat_id) - 1);
    w->chat_id[sizeof(w->chat_id) - 1] = '\0';
    w->next = NULL;
    w->timer = mg_timer_add(w->mgr, 30000, 0, long_poll_timeout_cb, w);

    pthread_mutex_lock(&data->lock);
    w->next = data->long_poll_head;
    data->long_poll_head = w;
    pthread_mutex_unlock(&data->lock);
...
static void webui_http_handler(struct mg_connection* c, int ev, void* ev_data) {
    if (ev == MG_EV_CLOSE) {
        ...
        if (data && c->data[0] == 'P') {
            ...
            while (cur) {
                if (cur->conn == c) {
                    if (cur->timer) mg_timer_free(&data->mgr.timers, cur->timer);
                    if (prev) prev->next = cur->next;
                    else data->long_poll_head = cur->next;
                    free(cur);
                    break;
