...
"function alignEnableToRight(){document.querySelectorAll('.list-item-card').forEach(card=>{const head=card.querySelector('.list-item-head');if(!head)return;const row=[...card.querySelectorAll('.list-kv')].find(x=>{const s=x.querySelector('span');return s&&s.textContent.trim()==='enabled'});if(!row||head.contains(row))return;head.appendChild(row)})}"
"function renderMenu(){"
"let h='';"
...
"async function renderOverview(){setTitle('概览','系统状态与关键配置摘要');pageRoot.innerHTML=card('系统状态','<div id=\"health\">Loading...</div>')+card('关键配置','<div id=\"cfgsum\">Loading...</div>');try{const h=await jget('/api/health');document.getElementById('health').innerHTML='<pre>'+esc(JSON.stringify(h,null,2))+'</pre>'}catch(e){document.getElementById('health').innerHTML='<div class=\"tag\">Error</div><div>'+esc(e.message)+'</div>'}try{const c=await jget('/api/config');const s={model:c.agent.model,apiBase:c.agent.apiBase,temperature:c.agent.temperature,max_tokens:c.agent.max_tokens,port:c.webui.port,log_level:c.log.level};document.getElementById('cfgsum').innerHTML='<pre>'+esc(JSON.stringify(s,null,2))+'</pre>'}catch(e){document.getElementById('cfgsum').innerHTML='<div class=\"tag\">Error</div><div>'+esc(e.message)+'</div>'}}"
"function markdownToHtml(md){let t=esc(String(md||'')).replace(/\\r\\n/g,'\\n');const blocks=[];t=t.replace(/```([\\w-]*)\\n([\\s\\S]*?)```/g,(_,lang,code)=>{const k='@@CODE'+blocks.length+'@@';blocks.push('<pre><code'+(lang?' data-lang=\"'+lang+'\"':'')+'>'+code+'</code></pre>');return k});t=t.replace(/^######\\s+(.+)$/gm,'<h6>$1</h6>').replace(/^#####\\s+(.+)$/gm,'<h5>$1</h5>').replace(/^####\\s+(.+)$/gm,'<h4>$1</h4>').replace(/^###\\s+(.+)$/gm,'<h3>$1</h3>').replace(/^##\\s+(.+)$/gm,'<h2>$1</h2>').replace(/^#\\s+(.+)$/gm,'<h1>$1</h1>');t=t.replace(/^>\\s+(.+)$/gm,'<blockquote>$1</blockquote>');t=t.replace(/\\*\\*([^*]+)\\*\\*/g,'<strong>$1</strong>').replace(/\\*([^*]+)\\*/g,'<em>$1</em>').replace(/`([^`]+)`/g,'<code>$1</code>');t=t.replace(/\\[([^\\]]+)\\]\\((https?:\\/\\/[^\\s)]+)\\)/g,'<a href=\"$2\" target=\"_blank\" rel=\"noopener noreferrer\">$1</a>');t=t.replace(/^(?:-|\\*)\\s+(.+)$/gm,'<li>$1</li>').replace(/(<li>.*<\\/li>\\n?)+/g,m=>'<ul>'+m.replace(/\\n/g,'')+'</ul>');t=t.split(/\\n{2,}/).map(x=>{const s=x.trim();if(!s)return'';if(/^<(h\\d|ul|ol|pre|blockquote)/.test(s))return s;return '<p>'+s.replace(/\\n/g,'<br>')+'</p>'}).join('');blocks.forEach((h,i)=>{t=t.replace('@@CODE'+i+'@@',h)});return t||'<p></p>'}"
...
static void handle_control_commands(struct mg_connection* c, WebUIChannelData* data) {
    cJSON* items = cJSON_CreateArray();
    const char* builtins[] = {"agent", "onboard", "gateway", "status", "channels"};
    for (size_t i = 0; i < sizeof(builtins) / sizeof(builtins[0]); i++) {
        cJSON* obj = cJSON_CreateObject();
        cJSON_AddStringToObject(obj, "name", builtins[i]);
        cJSON_AddStringToObject(obj, "source", "builtin");
...
static void handle_control_plugin_enable(struct mg_connection* c, struct mg_http_message* hm, WebUIChannelData* data) {
    ...
    if (strcmp(kind->valuestring, "channel") == 0) suffix = "_channel";
    else if (strcmp(kind->valuestring, "tool") == 0) suffix = "_tool";
    else if (strcmp(kind->valuestring, "command") == 0) suffix = "_command";
    if (!suffix) {
        cJSON_Delete(req);
        send_json(c, 400, "{\"error\":\"invalid_kind\"}");
        return;
    }
    size_t id_len = strlen(plugin_id->valuestring);
    size_t suffix_len = strlen(suffix);
    if (id_len <= suffix_len || strcmp(plugin_id->valuestring + id_len - suffix_len, suffix) != 0) {
        cJSON_Delete(req);
...
static void webui_send(Channel* self, OutboundMessage* msg) {
    if (!self || !msg) return;
    WebUIChannelData* data = (WebUIChannelData*)self->user_data;
    if (!data || !msg->channel.data || !msg->chat_id.data || !msg->content.data) return;
    if (strcmp(msg->channel.data, "webui") != 0) return;
    enqueue_reply(data, msg->chat_id.data, msg->content.data);
}
